...
class HistoricalDataBackfiller:
    ...
    def __init__(self, max_workers: int = 4, batch_size: int = 50):
        ...
        # 数据源配置
        self.data_sources = ['baostock', 'akshare', 'tencent']  # 优先级（Baostock第一）
    ...
    def fetch_daily_prices(self, symbol: str, start_date: str = '2010-01-01',
        ...
        """
        获取日线价格数据
        优先级: Baostock -> AKShare -> 腾讯财经 -> 模拟数据
        根据用户指示：Baostock（核心基本面）+ AKShare（补充行情/概念/爬虫财务）
        """
        if end_date is None:
            end_date = datetime.now().strftime('%Y-%m-%d')
        
        # 辅助函数：转换股票代码为Baostock格式
        ...
        try:
            ...
            print(f"  Baostock获取 {symbol} {start_date} 至 {end_date}...")
            
            # 转换股票代码格式
            baostock_symbol = format_symbol_for_baostock(symbol)
