"""Quick test: write 1 day of overnight_8step candidates - file output."""
...
out = open('test_write_result.txt', 'w', encoding='utf-8')
def log(msg):
...
for fp in DATA_DIR.glob("*.json"):
    ...
    try:
        ...
        pass
log(f"Loaded {len(all_klines)} stocks")

tds = get_trading_days_in_range(date(2026, 1, 1), date(2026, 1, 31))
log(f"Trading days: {len(tds)}")
...
log(f"Day {td}: {len(day_klines)} stocks with klines")

industries = load_industries()
log(f"Industries: {len(industries)}")
...
if picks:
    ...
    log(f"Items to write: {len(items)}")
    for it in items:
        log(f"  {it['ts_code']} score={it['final_score']:.1f}")

    log("Writing to DB...")
    t0 = time.time()
    n = write_candidates(items, td, source='overnight_8step', run_mode='afternoon')
    elapsed = time.time() - t0
    log(f"Written: {n} in {elapsed:.1f}s")
else:
    log("No picks")
