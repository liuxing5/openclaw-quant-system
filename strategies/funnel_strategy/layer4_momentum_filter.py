...
========================
决策逻辑：
  出现【需求吸收K线（EMA12附近锤子/刺通放量）】或
  【强势接力（昨日首板，今日回踩VWAP翘头）】；
  同时量比1.5~3；乖离率<6%；未出现天量上轨禁止信号。

吸收策略：⑤价格行为/VWAP/一进二改良/红三兵限利布林上轨反用/箱体反转K线
"""
...
# ── 线Python 计算（避开 pandas 开销，适合小数据集：──

def _fast_ema_last(values: np.ndarray, period: int) -> float:
    """快速计算EMA 最后一个值（Python：点数据比 pandas.ewm 快）"""
    n = len(values)
    if n == 0:
        return 0.0
...
def _batch_load_and_precompute(
    ...
    返回:
      ohlcv_cache: {ts_code: arr_dict}  —仅保留最否行用于K线形态检测
    ...
    for ts_code, group in df_all.groupby('ts_code', sort=False):
        ...
        # 转为 numpy 数组（纯 Python 计算，避免pandas 开销：
        ...
        # 保留最否5 行用亏K 线形态检测（转成 list of dict 方便并行处理：
        tail_rows = []
        start_idx = max(0, n - 5)
...
def _check_single(
    ...
    if not signal_found:
        result['reject_reason'] = '无买入信可K线形态不符'
        return result
...
def run_layer4_momentum_filter(
    ...
    if verbose:
        print(f"\n{'─'*60}")
        print(f"  [Layer 4] 动能与买入信号 —待筛通{n_total} 只)")
        print(f"{'─'*60}")
    ...
    if verbose:
        print(f"  ✅数据就绪: {n_loaded}/{n_total} 可(EMA/乖离/布林已预计算)")
    ...
    # 线Python 检查极快（~0.01ms/只），串行即可避免线程调度开销
