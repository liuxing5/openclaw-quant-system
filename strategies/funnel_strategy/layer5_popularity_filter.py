...
def _load_llm_candidates(trade_date: date, db_conn) -> Dict[str, dict]:
    """从daily_candidates 加载 LLM 多源候选（source='llm_multisource'）"""
    llm_map = {}
    try:
        cur = db_conn.cursor(cursor_factory=RealDictCursor)
        cur.execute("""
...
def run_layer5_popularity_filter(
    ...
    if verbose and (pe_reject > 0 or pb_reject > 0):
        print(f"  ⚠估值预筛淘汇 PE>{cfg.layer5_max_pe} ({pe_reject}可 + PB>{cfg.layer5_max_pb} ({pb_reject}可")
