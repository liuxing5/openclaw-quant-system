"""回测 - v8: 修复Layer1选股范围+信号衰减+涨停追涨保护（带超时）"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('d:/pythonProject/openclaw-quant-system/bt_output_v8.log', encoding='utf-8', mode='w'),
...
print(f'Running backtest v8 ({bt_cfg.start_date} ~ {bt_cfg.end_date})...', flush=True)
print(f'v8 fixes: Layer1涨幅优先选股+当日跌幅>5%过滤+信号衰减保护+涨停追涨保护', flush=True)
...
try:
    ...
    if result:
        with open('bt_result_v8.txt', 'w', encoding='utf-8') as f:
            f.write(result['summary'])
        if result['trades']:
            with open('bt_trades_v8.json', 'w', encoding='utf-8') as f:
