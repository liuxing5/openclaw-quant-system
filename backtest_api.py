"""
使用Supabase REST API获取数据并缓存，然后运行回测
"""
import requests
import pandas as pd
import numpy as np
...
def fetch_data_from_api(start_date, end_date, lookback_days=30):
    """使用Supabase REST API获取数据"""
    sd = datetime.strptime(start_date, "%Y-%m-%d")
    lookback_start = (sd - timedelta(days=lookback_days)).strftime("%Y-%m-%d")
    
    log(f"步骤1: 从Supabase API加载数据 {lookback_start} ~ {end_date}...")
    t0 = time.time()
    ...
    while True:
        url = f"{SUPABASE_URL}/rest/v1/daily_quotes"
        params = {
            "select": "ts_code,trade_date,open,high,low,close,volume,amount,pct_chg,turnover_rate",
            "trade_date": f"gte.{lookback_start},lte.{end_date}",
            "pct_chg": "not.is.null",
            "amount": "not.is.null",
            "turnover_rate": "not.is.null",
            "order": "ts_code.asc,trade_date.asc",
            "limit": limit,
            "offset": offset
        }
        ...
        try:
            ...
            if resp.status_code != 200:
                log(f"  API错误: {resp.status_code} - {resp.text}")
                break
...
def load_data_cached(start_date, end_date, lookback_days=30):
    ...
    return fetch_data_from_api(start_date, end_date, lookback_days)
...
def run_backtest(df, strategy_name, filter_func, actual_start_date=None):
    ...
    start_idx = 31  # MA20 + VOL_LOOKBACK + 1
    ...
    while i < len(trading_dates) - 1:
        ...
        buyable = candidates[candidates["close"] * 100 <= alloc].copy()
        if buyable.empty:
            i += 1
            continue
...
def print_report(trade_log, all_trades, final_capital, total_trades, win_trades, strategy_name):
    ...
    log(f"  总盈亏:     {final_capital - 1_000_000:>+12,.2f} 元")
    log(f"  总收益率:   {(final_capital / 1_000_000 - 1) * 100:>+11.2f}%")
    ...
    for entry in trade_log:
        cum_return = (entry["capital_after"] / 1_000_000 - 1) * 100
...
def main():
    ...
    try:
        ...
        log(f"  {'总收益率':<20} {(final_capital1/1_000_000-1)*100:>+14.2f}% {(final_capital2/1_000_000-1)*100:>+14.2f}%")
