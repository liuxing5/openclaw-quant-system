"""回测 - v7: 修复T+1+退出价格+移动止盈+时间止损+追涨保护"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('bt_output_v7.log', encoding='utf-8'),
...
meta_cfg = MetaStrategyConfig(
    ...
    layer6_trailing_activate_pct=0.08,     # 5%→8%：更多利润空间再启动移动止盈
    layer6_trailing_stop_pct=0.05,          # 3%→5%：从高点回撤5%才止盈，拿住主升浪
    layer6_max_holding_days=15,             # 10→15天：给慢牛更多时间
...
pm_cfg = PositionManagerConfig(
    ...
    trailing_activate_pct=0.08,             # 5%→8%
    trailing_stop_pct=0.05,                 # 3%→5%
    max_holding_days=15,                    # 10→15天
...
print(f'Running backtest v7 ({bt_cfg.start_date} ~ {bt_cfg.end_date})...', flush=True)
...
try:
    ...
    if result:
        with open('bt_result_v7.txt', 'w', encoding='utf-8') as f:
            f.write(result['summary'])
        if result['trades']:
            with open('bt_trades_v7.json', 'w', encoding='utf-8') as f:
