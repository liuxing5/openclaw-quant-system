...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('bt_output_v5.log', encoding='utf-8'),
...
# === v5 策略 ===
# v1是最好的: 3.28%收益, 0.65%回撤
# v5 = v1参数 + L0门槛降低 + 破位放量阈值提高

meta_cfg = MetaStrategyConfig(
    # L0: 降低门槛
    layer0_min_advancers=1200,  # v1=2000, v5=1200
...
pm_cfg = PositionManagerConfig(
    ...
    # 唯一改动: 提高破位放量量比阈值
    breakdown_vol_ratio_min=1.8,  # v1=1.2, v5=1.8
    high_vol_ratio_min=3.5,  # v1=3.0, v5=3.5
)
...
print(f'Running backtest v5 ({bt_cfg.start_date} ~ {bt_cfg.end_date})...', flush=True)
...
try:
    ...
    if result:
        with open('bt_result_v5.txt', 'w', encoding='utf-8') as f:
            f.write(result['summary'])
        if result['trades']:
            with open('bt_trades_v5.json', 'w', encoding='utf-8') as f:
