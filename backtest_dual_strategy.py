...
STOP_LOSS = -0.02  # -2%止损
MIN_LOTS = 100     # 最少100股
MAX_PICKS_PER_DAY = 5  # 每天最多选5只

# 低吸策略参数
DIP_PCT_LO = -6.0
DIP_PCT_HI = -3.0
DIP_VOL_RATIO_MAX = 0.8  # 缩量下跌
DIP_MA20_TOLERANCE = 0.05  # MA20附近±5%
DIP_MA5_DEVIATION = 0.03   # 偏离MA5超过3%
DIP_TURN_MIN = 3.0
DIP_TURN_MAX = 15.0
DIP_MIN_AMOUNT = 30_000_000

# 右侧企稳策略参数
STABILIZE_PCT_MIN = 2.0    # 阳线涨幅>2%
STABILIZE_VOL_RATIO_MIN = 1.5  # 放量
...
def load_data(start_date, end_date, lookback_days=30):
    ...
    # 分批查询：按月分批，避免transaction pooler超时
    from datetime import datetime as dt
    start_dt = dt.strptime(lookback_start, "%Y-%m-%d")
    ...
    while current <= end_dt:
        ...
        date_ranges.append((current.strftime("%Y-%m-%d"), min(month_end, end_dt).strftime("%Y-%m-%d")))
        ...
        for attempt in range(5):
            ...
            try:
                ...
                if rows:
                    ...
                    total += len(rows)
                log(f"  {batch_start}~{batch_end}: {len(rows)} 条 (累计 {total})")
                time.sleep(1)  # 等待连接池释放
                break
                ...
                if conn:
                    ...
                    conn = None
                if attempt < 4:
                    log(f"  {batch_start}~{batch_end} 失败, 重试...")
                    time.sleep(3)
                else:
                    log(f"  {batch_start}~{batch_end} 失败: {str(e)[:80]}")
...
def main():
    ...
    try:
        ...
        df = load_data("2025-05-01", "2025-08-31")
        log("  开始计算指标...")
